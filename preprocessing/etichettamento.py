...
outputFileClass = open(nome + "_C.csv", "w")
outputFileOrder = open(nome + "_O.csv", "w")
outputFileFamily = open(nome + "_F.csv", "w")
outputFileGenus = open(nome + "_G.csv", "w")

matrix = list(open(sys.argv[1], "r"))
headers = matrix[:1]
matrix = matrix[1:]
for f in [outputFileClass, outputFileOrder, outputFileFamily, outputFileGenus]:
    f.write(headers[0] + ",classification\n")
